...
typedef struct _CustomData {
  ...
  GstElement *pl_cam,*pl_send_cam, *pl_receive_cam,*pl_receive_cam1,*pl_receive_cam2 ; 
...
//----------------------------------MONITOR --AND--RECORD-- MY --CAMERA--------------------------------------------------------------------------------------

  data.pl_cam = gst_parse_launch("v4l2src ! autovideosink ", &error);
  //if(!data.pl_cam)g_printerr ("Camera could not start\n");

//----------------------------------STREM ---- MY --CAMERA--TO PEER------------------------------------------------------------------------------------



//send video streammmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm
/*gst-launch-1.0 v4l2src ! image/jpeg, width=1280, height=720 ! rtpjpegpay ! udpsink host=127.0.0.1 port=5001   */
//receiver video streammmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm
/*gst-launch-1.0  udpsrc port=5001 ! application/x-rtp, encoding-name=JPEG, payload=26 ! rtpjpegdepay ! jpegparse ! jpegdec ! videoconvert ! xvimagesink sync= false           */
//	gst-launch-1.0 -v udpsrc port=7150 caps = "application/x-rtp, media=(string)video, clock-rate=(int)90000, encoding-name=(string)H264, payload=(int)96" ! rtph264depay ! decodebin ! videoconvert ! 		autovideosink;

	data.pl_send_cam = gst_parse_launch("v4l2src ! video/x-raw,framerate=20/1 ! videoscale ! videoconvert ! x264enc tune=zerolatency bitrate=500 speed-preset=superfast ! rtph264pay ! multiudpsink clients=195.251.253.201:8000,195.251.253.204:8000 ", &error);
	data.pl_receive_cam1 = gst_parse_launch("udpsrc port=8000 ! application/x-rtp, media=(string)video, payload=(int)96 ! rtph264depay ! decodebin ! videoconvert ! autovideosink", &error);
	data.pl_receive_cam2 = gst_parse_launch("udpsrc port=8001 ! application/x-rtp, media=(string)video, payload=(int)96 ! rtph264depay ! decodebin ! videoconvert ! autovideosink", &error);
		
	//gst_element_set_state (data.pl_send_cam, GST_STATE_PLAYING); //first start to send stream to peer
	//gst_element_set_state (data.pl_receive_cam, GST_STATE_PLAYING); //first start to send stream to peer
...
//                                
//-------alsasrc-----------VOLUME---------AUDIOCONVERT---------pulsesink(LISTEN YOUR SOUND)
  ...
  data.audio_sink1 = gst_element_factory_make("pulsesink", "audio_sink1");
...
//-----------------------------alsasrc--------AUDIOCONVERT STEREO TO MONO----------WAVENC---------FILESINK(WRITE TO FILE)
  ...
  data.audio_source7 = gst_element_factory_make("pulsesrc", "audio_source7");
...
//				  
  ...
  data.audio_source2 = gst_element_factory_make("pulsesrc", "audio_source2");
  ...
  if (!data.pipeline2 || !data.audio_source2 || !data.capsfilter2 || !data.audio_convert2 || !data.audio_rtp2 || !data.audio_udp2  ) {
    g_printerr ("Not all pl2 elements could be created.\n");
    return ;
  }
  gst_bin_add_many (GST_BIN (data.pipeline2), data.audio_source2, data.capsfilter2,  data.audio_convert2, data.audio_rtp2, data.audio_udp2, NULL); //
  if (!gst_element_link_many (data.audio_source2, data.capsfilter2,  data.audio_convert2, data.audio_rtp2, data.audio_udp2,NULL)) {
    g_printerr ("Elements in pl2 could not be linked.\n");
...
//                                                           QUEUE------VOLUME-----pulsesink(LISTEN PEER'S SOUND)
...
//				                             QUEUE------WAVENC-----FILESINK(WRITE TO FILE)
  ...
  g_object_set (data.audio_source3 , "port",7150 , NULL);
  //gst_caps_unref(data.caps3);
  ...
  data.audio_sink3 = gst_element_factory_make("pulsesink", "audio_sink3");
...
//                                                           
//-------UDPSRC------RTPL16DEPAY---AUDIOCONVERT-----VOLUME-----pulsesink(LISTEN PEER'S SOUND)
//				                             

 // data.caps = gst_caps_from_string("application/x-rtp,media=audio, clock-rate=88200, encoding-name=L16, payload=96");
  data.audio_source9 = gst_element_factory_make("udpsrc", "audio_source9");
  g_object_set (data.audio_source9 , "port",7150 , NULL);
  ...
  data.audio_sink9 = gst_element_factory_make("pulsesink", "audio_sink9");
...
//				                             
  ...
  g_object_set (data.audio_source10 , "port",7151 , NULL); //"caps", data.caps,
  //gst_caps_unref(data.caps);
  ...
  data.audio_sink10 = gst_element_factory_make("pulsesink", "audio_sink10");
...
//                               
//-------alsasrc-----------OPUSENC----OPUSDEC----pulsesink(LISTEN YOUR SOUND AFTER OPUS ENCODE DECODE)
//				 




   data.audio_source4 = gst_element_factory_make("pulsesrc", "audio_source4");
  ...
  data.audio_sink4 = gst_element_factory_make("pulsesink", "audio_sink4");
...
//-------------------------BUILD THE PIPELINE8------START---------YOUR DIRECT SOUND----RECORDED-------------------------------
   ...
   data.audio_source8 = gst_element_factory_make("pulsesrc", "audio_source8");
...
//-------alsasrc---------opusenc-----rtpopuspay-------UDPSINK(SEND TO NETWORK)
  ...
  data.audio_source5 = gst_element_factory_make("pulsesrc", "audio_source5");
...
//                                                           QUEUE------VOLUME-----pulsesink(LISTEN PEER'S SOUND AND SEND BACK)
...
//				                             QUEUE------WAVENC-----FILESINK(WRITE TO FILE)
  ...
  data.audio_sink6 = gst_element_factory_make("pulsesink", "audio_sink6");
...
//                                                           QUEUE------VOLUME-----pulsesink(MY----STREAM FROM PEER)
//-------UDPSRC------rtpopusdepay---opusdec----TEE------|
//				                             QUEUE------WAVENC-----FILESINK(WRITE TO FILE)
  ...
  data.audio_sink11 = gst_element_factory_make("pulsesink", "audio_sink11");
...
static void sample_rate (GtkButton *button, GstElement *source, GObject *context_object ) //, GObject *context_object
{
	...
	data.caps3 = gst_caps_new_simple("audio/x-raw",
                                        //"format", G_TYPE_STRING, "S16LE",
                                        "rate", G_TYPE_INT, sam_rate,
	                	        //"layout", G_TYPE_STRING, "INTERLEAVED",
                                        //"channels", G_TYPE_INT, 2, 
				        NULL );
	data.caps4 = gst_caps_new_simple("application/x-rtp",
                                         "media", G_TYPE_STRING, "audio",
                   			 "payload",G_TYPE_INT,96,
                    			 "clock-rate", G_TYPE_INT, 2*sam_rate,
                      			NULL);
  	g_object_set (data.capsfilter2 , "caps", data.caps3, NULL);
	g_object_set (data.audio_source3 ,"caps", data.caps4 , NULL);
	g_object_set (data.audio_source10 ,"caps", data.caps4 , NULL);
	g_object_set (data.audio_source9 ,"caps", data.caps4 , NULL);
        g_print ("Sample-rate: %"G_GUINT64_FORMAT" Hz\n", sam_rate);
...
static void play_cb1 (GtkToggleButton *tog, gpointer user_data) {
	...
	if (gtk_toggle_button_get_active (tog))
	{
  		if  (pcm == 0 && opus == 1 )
		{
			...
			char command[100];
   			strcpy( s_rate, s_r);
   			strcpy( command, "pactl load-module module-loopback latency_msec=1 rate=" );
...
static void play_cam (GtkToggleButton *tog, gpointer user_data) {

	if (gtk_toggle_button_get_active (tog)){
        	g_print ("trying to start camera");
...
static void stream_my_cam (GtkToggleButton *tog, gpointer user_data) {
	if (gtk_toggle_button_get_active (tog)){
        g_print ("Stream my camera to server");
  		gst_element_set_state (data.pl_send_cam, GST_STATE_PLAYING);
	     }
  	else {
...
static void open_peers_cam (GtkToggleButton *tog, gpointer user_data) {
	if (gtk_toggle_button_get_active (tog)){
  		...
  		gst_element_set_state (data.pl_receive_cam1, GST_STATE_PLAYING);
  		gst_element_set_state (data.pl_receive_cam2, GST_STATE_PLAYING);

	     }
  	else {
        	g_print ("Stops peers camera");
  		gst_element_set_state (data.pl_receive_cam1, GST_STATE_PAUSED);
  		gst_element_set_state (data.pl_receive_cam2, GST_STATE_PAUSED);
  	}
...
//-----------------test with toggle 5-11-2019-------------------
  ...
  g_signal_connect (GTK_TOGGLE_BUTTON(button), "toggled", G_CALLBACK (play_cam), NULL);
...
//-============================================================================
  ...
  g_signal_connect (GTK_TOGGLE_BUTTON(button), "toggled", G_CALLBACK (stream_my_cam), NULL);
