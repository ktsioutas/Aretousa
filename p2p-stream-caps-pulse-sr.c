...
//----------------------------------MONITOR --AND--RECORD-- MY --CAMERA--------------------------------------------------------------------------------------
  ...
  data.pl_cam = gst_parse_launch("v4l2src ! tee name=t ! queue ! xvimagesink t. ! queue ! videoconvert ! x264enc ! flvmux ! filesink location=test.flv ", &error);
...
//sender video streammmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm
...
/*gst-launch-1.0  udpsrc port=5001 ! application/x-rtp, encoding-name=JPEG, payload=26 ! rtpjpegdepay ! jpegparse ! jpegdec ! videoconvert ! xvimagesink sync= false           */
	...
	data.pl_send_cam = gst_parse_launch("v4l2src ! image/jpeg, width=1280, height=720 ! rtpjpegpay ! udpsink host=83.212.101.51 port=2000", &error);
	data.pl_receive_cam = gst_parse_launch("udpsrc port=7151 ! application/x-rtp, encoding-name=JPEG, payload=26 ! rtpjpegdepay ! jpegparse ! jpegdec ! videoconvert ! xvimagesink sync= false", &error);
...
//-------ALSASRC-----------VOLUME---------AUDIOCONVERT---------ALSASINK(LISTEN YOUR SOUND)
  ...
  data.audio_source1 = gst_element_factory_make("pulsesrc", "audio_source1");
...
//                                
//-------PULSESRC---------AUDIOCONVERT-----RTPL16PAY-------UDPSINK(SEND TO PEER)
//				  
...
//-----------------------camera-----------buttons------------------------------------
  ...
  g_signal_connect (button, "clicked", G_CALLBACK (play_cam), NULL);
  ...
  g_signal_connect (button, "clicked", G_CALLBACK (stream_my_cam), NULL);
  
  button = gtk_builder_get_object (builder, "button12");
  g_signal_connect (button, "clicked", G_CALLBACK (open_peers_cam), NULL);
